...
import styles from "./order-new-account.module.scss";
...
export default function NewAccount() {
  ...
  // Get stored user_id, phone, shopId, and tableNumber from localStorage
  useEffect(() => {
  ...
  const handleVerify = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (ok) {
        ...
        // Smart redirect based on restaurant context
        ...
        if (shopId) {
          if (tableNumber) {
            // Table flow
            console.log(
              `✅ Profile complete + Table ${tableNumber} → /restaurant/${shopId}?table=${tableNumber}`
            );
            router.push(`/restaurant/${shopId}?table=${tableNumber}`);
          } else {
            // Bar flow
            console.log(
              `✅ Profile complete + Bar flow → /restaurant/${shopId}`
            );
