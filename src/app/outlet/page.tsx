...
const OutletPage: React.FC = () => {
  ...
  const handleButtonClick = () => {
    ...
    const AdsRef = useRef<HTMLDivElement>(null);
       ...
       useEffect(() => {
    const handleScroll = () => {
      const bannerBottom = bannerRef.current
        ? bannerRef.current.offsetTop + bannerRef.current.offsetHeight
        : 0;
      const adsBottom = AdsRef.current
        ? AdsRef.current.offsetTop + AdsRef.current.offsetHeight
        : 0;
      const scrollY = window.scrollY;

      // 🧭 Header color toggle (white after banner)
      if (scrollY > bannerBottom - 100) {
        setHeaderWhite(true);
      } else {
        setHeaderWhite(false);
      }

      // 🧭 Show category bar after ads section
      if (scrollY > adsBottom - 120) {
    ...
    {/* 🧠 Dynamic Header */}
      <header
        className={classNames(
          ...
          'header',
          {
            'bg-white': headerWhite,
            'bg-transparent': !headerWhite,
          }
        )}
      >
        <button
        ...
        <div
          className={`${styles.outletTitle} pageTitle transition-colors duration-300 ${
            headerWhite ? 'text-black opacity-100' : 'text-white opacity-0'
          }`}
        >
      ...
      <section className='page_content padding_top'>
        ...
        <div className={`${styles.outletName}`}>
            Welcome to the Vertige Investment Group Annual Summit
        </div>
        <div ref={AdsRef} className={`${styles.outletAds}`}>
            <div>
                <h5>Sign in for a smoother checkout experience!</h5>
                <Link className={`${styles.signInBtn}`} href='/login'>Take me there</Link>
            </div>
        ...
        {/* 🧭 Category bar */}
        {/* <div className={`${styles.catMenu} mt-4 bg-white border-b-4 border-gray-200 overflow-x-auto no-scrollbar`}>
          <div className="flex">
            {categories.map((cat) => (
              <button
                key={cat.id}
        ...
        {/* 🧭 Category bar (show/hide on scroll) */}
        <div
          className={classNames(
            styles.catMenu,
...
/>
        ...
        {cartCount > 0 && (
          <div className={styles.bottomButton}>
            <Link
              href="/choose"
              className="w-full bg-primary text-white py-2 rounded-lg flex justify-between px-4"
            >
              <span>
